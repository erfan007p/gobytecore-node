...
describe('GoByte Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.gobytecore/data/gobyted')
  var nodesConf = [
    {
  ...
  it('step 1: will connect to three gobyted daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = gobytecore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.gobyted.once('tip', function(height) {
