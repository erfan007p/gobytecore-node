...
/**
 * Will remove a service from gobytecore-node.json
...
/**
 * Will remove the Node.js service and from the gobytecore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The gobytecore-node configuration directory
...
function remove(options, done) {
  ...
  var gobytecoreConfigPath = path.resolve(configPath, 'gobytecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(gobytecoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a gobytecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to gobytecore-node.json
        removeConfig(gobytecoreConfigPath, service, next);
      });
    }, done
  );
}
