...
/**
 * Will return the path and default gobytecore-node configuration. It will search for the
 * configuration file in the "~/.gobytecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.gobytecore');
  var defaultConfigFile = path.resolve(defaultPath, './gobytecore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['gobyted', 'web'];
