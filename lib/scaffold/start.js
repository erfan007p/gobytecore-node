...
var GoBytecoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for gobyted is necessary for this upgrade with the "reindex=1" gobyte.conf option. \n' +
                 'There are changes necessary in both gobyte.conf and gobytecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/gobytecoin/gobytecore-node/blob/gobyted/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed gobytecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './gobytecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in gobytecore-node services directory
    ...
    if (servicePackage.gobytecoreNode) {
      serviceModule = service.name + '/' + servicePackage.gobytecoreNode;
    }
 ...
 * @param {Node} node - The GoBytecore Node instance
 */
