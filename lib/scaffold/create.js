...
var BASE_PACKAGE = {
  description: 'A full GoByte node build with GoBytecore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@gobytecoin/gobytecore-lib': '^' + gobytecore.version,
    '@gobytecoin/gobytecore-node': version
  }
};

/**
 * Will create a directory and gobyte.conf file for GoByte.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createGoByteDirectory(datadir, done) {
...
/**
 * Will create a base GoBytecore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The gobyte database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/gobytecore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a GoBytecore Node directory, configuration file,
 * gobyte configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the gobytecore node configuration directory
 * @param {String} options.datadir - The path to the gobyte datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the gobytecore-node directory and configuration
    ...
    function(next) {
      // Setup the GoByte directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createGoByteDirectory(absDataDir, next);
      } else {
        next();
      }
    },
