...
var Proposal = gobytecore.GovObject.Proposal;
 ...
 */
function GoByte(options) {
  if (!(this instanceof GoByte)) {
    return new GoByte(options);
  }
...
util.inherits(GoByte, Service);

GoByte.dependencies = [];

GoByte.DEFAULT_MAX_TXIDS = 1000;
GoByte.DEFAULT_MAX_HISTORY = 50;
GoByte.DEFAULT_MAX_UTXO = 1000;
GoByte.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
GoByte.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
GoByte.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
GoByte.DEFAULT_SPAWN_RESTART_TIME = 5000;
GoByte.DEFAULT_SPAWN_STOP_TIME = 10000;
GoByte.DEFAULT_TRY_ALL_INTERVAL = 1000;
GoByte.DEFAULT_REINDEX_INTERVAL = 10000;
GoByte.DEFAULT_START_RETRY_INTERVAL = 5000;
GoByte.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
GoByte.DEFAULT_TRANSACTION_CONCURRENCY = 5;
GoByte.DEFAULT_INSTANTSEND_FEE = 10000;
GoByte.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'gobytecore'
};

GoByte.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || GoByte.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || GoByte.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || GoByte.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || GoByte.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || GoByte.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || GoByte.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || GoByte.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || GoByte.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || GoByte.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || GoByte.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || GoByte.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

GoByte.prototype._initCaches = function() {
...
GoByte.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
GoByte.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
GoByte.prototype.getPublishEvents = function() {
  return [
    {
...
GoByte.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'gobyted/' + name, 'total:', this.subscriptions[name].length);
};

GoByte.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'gobyted/' + name, 'total:', this.subscriptions[name].length);
};

GoByte.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (gobytecore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'gobyted/addresstxid', 'total:', _.size(this.subscriptions.address));
};

GoByte.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'gobyted/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
GoByte.prototype.unsubscribeAddressAll = function(emitter) {
...
GoByte.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = GoByte.DEFAULT_CONFIG_SETTINGS;
...
GoByte.prototype._parseGoByteConf = function(configPath) {
  var options = {};
...
GoByte.prototype._expandRelativeDatadir = function() {
...
GoByte.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using GoByte config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseGoByteConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseGoByteConf(networkConfigPath));
  }
...
GoByte.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of gobytecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of gobytecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of gobytecore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to gobyted from gobytecore. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in gobyte.conf'
  );
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that gobyted is undergoing a reindex. ' +
             ...
             'of gobytecore-node services will start.');
    node._reindex = true;
  }
};

GoByte.prototype._resetCaches = function() {
 ...
 * @private
 */
GoByte.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
GoByte.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

GoByte.prototype._initChain = function(callback) {
  var self = this;
...
GoByte.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 12454
  };
  if (this.node.network === gobytecore.Networks.testnet) {
    networkOptions.rpcport = 13454;
  }
  return networkOptions;
};

GoByte.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === gobytecore.Networks.testnet) {
...
GoByte.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === gobytecore.Networks.testnet) {
...
GoByte.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('gobyted/hashblock', message.toString('hex'));
    }
  }

};

GoByte.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
GoByte.prototype._updateTip = function(node, message) {
  var self = this;
...
GoByte.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
GoByte.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('gobyted/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
GoByte.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = gobytecore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('gobyted/rawtransaction', message.toString('hex'));
    }

    var tx = gobytecore.Transaction();
    tx.fromString(message);
    var txid = gobytecore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

GoByte.prototype._zmqTransactionLockHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('gobyted/transactionlock', message.toString('hex'));
    }

  }
};

GoByte.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || GoByte.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

GoByte.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
GoByte.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
GoByte.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  }, node._reindexWait || GoByte.DEFAULT_REINDEX_INTERVAL);

};

GoByte.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
GoByte.prototype._stopSpawnedGoByte = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
...
GoByte.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedGoByte(function(err) {
    if (err) {
      return callback(err);
    }
...
GoByte.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
GoByte.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('GoByte configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
GoByte.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
GoByte.prototype.syncPercentage = function(callback) {
  var self = this;
...
GoByte.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, gobytecore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
GoByte.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = gobytecore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
GoByte.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
GoByte.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
GoByte.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
GoByte.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in gobytecore is most recent to less recent
 ...
 * @param {Function} callback
 */
GoByte.prototype.getAddressTxids = function(addressArg, options, callback) {
...
GoByte.prototype._getConfirmationsDetail = function(transaction) {
...
GoByte.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
GoByte.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
GoByte.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Function} callback
 */
GoByte.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
GoByte.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof gobytecore.Address) {
...
GoByte.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
GoByte.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
GoByte.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
GoByte.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a GoBytecore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
GoByte.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = gobytecore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
GoByte.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
GoByte.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
GoByte.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
GoByte.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
GoByte.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a GoBytecore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
GoByte.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
GoByte.prototype.govObjectList = function(options, callback) {
  var self = this;
...
GoByte.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
GoByte.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
GoByte.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
GoByte.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
GoByte.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
GoByte.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
GoByte.prototype.govObjectInfo = function (callback) {
  var self = this;
...
GoByte.prototype.govCount = function (callback) {
  var self = this;
...
GoByte.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
GoByte.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
GoByte.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
GoByte.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
GoByte.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getBestChainLock = function (callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.getInfo = function(callback) {
  var self = this;
...
GoByte.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GoByte.prototype.stop = function(callback) {
